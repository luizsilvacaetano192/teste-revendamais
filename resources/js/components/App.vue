	 ...
	 //   },
        ...
        methods: {
           async getZipCodeData(zipCode) {
                const response = await fetch('http://127.0.0.1:8000/api/v1/addresses?zip_code=' + zipCode);
            ...
            async getfuzzySearchData(fuzzySearch) {
                const response = await fetch('http://127.0.0.1:8000/api/v1/addresses/search?fuzzySearch=' + fuzzySearch);
                const data = await response.json();   
                if(response.ok){
                    this.objSearch = data;
